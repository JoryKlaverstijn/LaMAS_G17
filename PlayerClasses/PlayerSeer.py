...
class PlayerSeer(Player):
    ...
    def choose_player_to_reveal(self, km, alive_player_ids):
        ...
        for idx in targets:
            if not km.knows_wolf(self.id, idx) and km.suspects(self.id, idx):
                return idx

        # Otherwise we identify a random person that we don't know the identity of
        for idx in targets:
            if not km.knows_wolf(self.id, idx) and not km.knows_little_girl(self.id, idx)\
                    and not km.knows_good(self.id, idx):
                return idx
